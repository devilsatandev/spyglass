...
const App: React.FC = () => {
  ...
  const handleAnalyze = useCallback(async (competitors: string[]) => {
    ...
    try {
      const report = await generateAnalysisPlan(validCompetitors);
  ...
  const handleClearHistory = useCallback(() => {
    if (window.confirm("Tem certeza de que deseja limpar todo o histórico de análises? Esta ação não pode ser desfeita.")) {
      setAnalysisHistory([]);
      setAnalysisReport(null); // Also clear the current view
    }
  }, []);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-12">
            ...
            <main className="lg:col-span-8 xl:col-span-9">
              <section id="input-section">
                <CompetitorInputForm onAnalyze={handleAnalyze} isLoading={isLoading} />
              </section>
